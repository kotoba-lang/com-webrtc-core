(ns webrtc_core.main
  "Kotodama WASM entrypoint for the Webrtc_core clean-room actor (L4) — Clojure port.
  ...
  facts (`webrtc_core.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "webrtc_core")
(def tier "L4")
...
(def entity-specs
  [{:entity "RTCPeerConnection" :plural "rtcpeerconnections" :id-prefix "webrtcco_rtc"
    :fields [:signalingState :iceConnectionState :iceGatheringState :connectionState] :required [:signalingState :iceConnectionState]
    :coerce {} :refs {}}
   {:entity "RTCDataChannel" :plural "rtcdatachannels" :id-prefix "webrtcco_rtc"
    :fields [:label :ordered :protocol :negotiated :readyState :bufferedAmount] :required [:label :ordered]
    :coerce {:ordered :bool :negotiated :bool :readyState :string :bufferedAmount :int} :refs {}}
   {:entity "RTCSessionDescription" :plural "rtcsessiondescriptions" :id-prefix "webrtcco_rtc"
    :fields [:type :sdp] :required [:type :sdp]
    :coerce {} :refs {}}
   {:entity "RTCIceCandidate" :plural "rtcicecandidates" :id-prefix "webrtcco_rtc"
    :fields [:candidate :sdpMLineIndex :sdpMid :protocol :port :address :type :tcpType :priority] :required [:candidate :sdpMLineIndex]
    :coerce {:sdpMLineIndex :int :port :int :priority :int} :refs {}}
   {:entity "RTCRtpTransceiver" :plural "rtcrtptransceivers" :id-prefix "webrtcco_rtc"
    :fields [:mid :direction :currentDirection] :required [:mid :direction]
    :coerce {} :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"webrtc_core.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "webrtc_core-compat" :tier tier :entities entities} 200])
...
(defn start! [] :webrtc_core-compat/ready)
